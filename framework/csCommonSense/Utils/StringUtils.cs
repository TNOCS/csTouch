...
namespace csShared.Utils
{
  ...
  {
    internal const char WildcardString = '*';
    ...
    public static int CompareAlphaNumeric(string a, string b)
    {
      ...
      if (ans == 0)
      {
        if (a != null) return a.CompareTo(b);
      }
      return ans;
    }
    ...
    public static bool WildcardEquals(string pattern, string str, bool ignoreCase)
    {
      ...
      if (pattern == null || str == null)
        return false;
      if (pattern.Length == 1 && pattern[0] == WildcardString)
        return true;
      if (str == String.Empty && pattern != String.Empty)
        return false;
      if (pattern.IndexOf(WildcardString) == -1 && pattern.IndexOf(WildcardChar) == -1)
        return pattern == str;
      return WildcardEquals(pattern, 0, str, 0);
    }

    // Copied from beagled/Lucene.Net/Search/WildcardTermEnum.cs
    internal static bool WildcardEquals(string pattern, int patternIdx, string stringRenamed, int stringIdx)
    {
      int p = patternIdx;
      ...
      {
        // End of string yet?
        bool sEnd = (s >= stringRenamed.Length);
        // End of pattern yet?
        ...
        if (sEnd)
        {
          ...
          while (wildcardSearchPos < pattern.Length && justWildcardsLeft)
          {
            ...
            if (wildchar != WildcardChar && wildchar != WildcardString)
            {
        ...
        if (pattern[p] == WildcardChar)
        {
          continue;
        }

        //
        if (pattern[p] == WildcardString)
        {
          // Look at the character beyond the '*'.
          ++p;
          // Examine the string, starting at the last character.
          for (int i = stringRenamed.Length; i >= s; --i)
          {
            if (WildcardEquals(pattern, p, stringRenamed, i))
            {
              return true;
            }
          }
